...
class BaseICDSAggregationHelper(object):
    ...
    def ucr_tablename(self):
        return self._ucr_tablename(self.ucr_data_source_id)
...
class AggAwcHelper(BaseICDSAggregationHelper):
    ...
    def __init__(self, month):
        self.month = transform_day_to_month(month)
        self.month_end = self.month + relativedelta(months=1, seconds=-1)
        self.start_date = month_formatter(self.month)
        self.end_date = month_formatter(self.month_end)
        self.prev_month = month_formatter(self.month - relativedelta(months=1))
        self.month_start_6m = month_formatter(self.month - relativedelta(months=6))
        self.month_end_11yr = month_formatter(self.month_end - relativedelta(years=11))
        self.month_start_15yr = month_formatter(self.month - relativedelta(years=15))
        self.month_end_15yr = month_formatter(self.month_end - relativedelta(years=15))
        self.month_start_18yr = month_formatter(self.month - relativedelta(years=18))
    ...
    def updates(self):
        ...
        """.format(
            tablename=self.tablename,
            daily_attendance='daily_attendance_{}'.format(month_formatter(self.month)),
            tmp_table=uuid.uuid4().hex
        ), {
            'start_date': self.start_date
        }
        ...
        """.format(
            tablename=self.tablename,
            child_health_monthly="child_health_monthly_{}".format(self.start_date),
            tmp_table=uuid.uuid4().hex
        ), {}
        ...
        """.format(
            tablename=self.tablename,
            ccs_record_monthly="ccs_record_monthly_{}".format(self.start_date),
            tmp_table=uuid.uuid4().hex
        ), {}
        ...
        yield """
         UPDATE "{tablename}" agg_awc SET
           ...
           FROM (SELECT DISTINCT(awc_id)
              FROM agg_awc
         WHERE month = %(prev_month)s AND usage_num_hh_reg > 0 AND awc_id <> 'ALL') ut
         WHERE ut.awc_id = agg_awc.awc_id;
        """.format(
            tablename=self.tablename
        ), {
